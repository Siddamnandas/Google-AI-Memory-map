    ...
    <div className="p-4 max-w-lg mx-auto">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-lg space-y-2">
            {userOrder.map((event, index) => {
                const isCorrect = isFinished && event.id === correctOrder[index].id;
                const isIncorrect = isFinished && event.id !== correctOrder[index].id;

                let stateClass = 'bg-gray-50';
                if (isFinished) {
                    stateClass = `animate-fade-in ${isCorrect ? 'bg-green-100' : 'bg-red-100'}`;
                } else if (hintedIndex === index) {
                    stateClass = 'bg-yellow-200 ring-2 ring-yellow-500';
                }

                return (
                    <div 
                        key={event.id} 
                        className={`flex items-center justify-between p-3 rounded-lg transition-all duration-300 ${stateClass}`}
                        style={isFinished ? { animationDelay: `${index * 100}ms`, opacity: 0, animationFillMode: 'forwards' } : {}}
                    >
        ...
        {isFinished ? (
            <div 
                ...
                style={{ animationDelay: `${userOrder.length * 100 + 200}ms`, opacity: 0, animationFillMode: 'forwards' }}
            >

...
import { generateImageFromPrompt, generateImageFromImageAndPrompt, analyzeImageWithPrompt, generateCode } from '@/services/geminiService';
...
const AIStudio: React.FC<AIStudioProps> = ({ user }) => {
    ...
    const [codePrompt, setCodePrompt] = useState('Create a simple React component that shows a button and a counter that increments on click.');
    ...
    // Update editor value when generated code changes
    useEffect(() => {
        if (monacoInstanceRef.current) {
            monacoInstanceRef.current.setValue(generatedCode);
        }
    }, [generatedCode]);
    ...
    const handleGenerateCode = async () => {
        ...
        setGeneratedCode(result || '// Failed to generate code.');
    ...
    return (
        <div className="pb-28">
            ...
            {/* Code Lab Module */}
            <div className="px-4 pt-8">
                <h2 className="text-warm-gray text-lg font-bold">Code Lab</h2>
                <p className="text-warm-gray text-base font-normal">Generate code snippets with AI.</p>
                <div className="mt-4 space-y-4">
                    <textarea
                        className="w-full rounded-lg border-gray-200 p-4 text-warm-gray placeholder-warm-gray/50 focus:ring-2 focus:ring-coral-pink shadow-sm disabled:bg-gray-100"
                    ...
                    <div className="flex items-center gap-4">
                        <select
                            ...
                            className="flex-grow rounded-lg border-gray-200 p-3 text-warm-gray focus:ring-2 focus:ring-coral-pink shadow-sm bg-white"
                            disabled={isGeneratingCode}
                        >
                        ...
                        <button
                            ...
                            className="flex-shrink-0 flex items-center justify-center gap-2 py-3 px-6 bg-coral-pink text-white font-bold rounded-full shadow-lg hover:bg-coral-pink/90 transition disabled:bg-coral-pink/50 disabled:cursor-wait"
                        >
                            {isGeneratingCode ? (
                                <>
                                    <div className="w-5 h-5 border-2 border-dashed rounded-full animate-spin border-white"></div>
                    ...
                    <div className="relative aspect-video w-full rounded-lg bg-[#1e1e1e] overflow-hidden shadow-inner" ref={editorRef}>

  ...
  if (currentQuestionIndex >= questions.length) {
    return (
      <div className="text-center p-8 bg-white rounded-xl shadow-md animate-fade-in">
        ...
        <p className="text-5xl font-bold text-mint-500 my-4">{finalScore}</p>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg">
            ...
            <div className="space-y-3">
                {currentQuestion.options.map((option, index) => {
                    ...
                    return (
                        <button key={index} onClick={() => handleOptionClick(option)} disabled={isAnswered} className={buttonClass}>
                           {option}
                        </button>
                    );
                })}
            ...
            {isAnswered && (
                <button onClick={handleNextQuestion} className="w-full mt-6 py-3 bg-coral-500 text-white font-bold rounded-full shadow-lg hover:bg-coral-600">
                    Next Question
                </button>
            )}
        </div>
